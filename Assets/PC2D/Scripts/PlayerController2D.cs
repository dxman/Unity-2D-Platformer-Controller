...
namespace PC2D
{
    ...
    public class PlayerController2D : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (_motor.motorState == PlatformerMotor2D.MotorState.FreedomState)
            {
                _motor.normalizedXMovement = UnityEngine.Input.GetAxis(PC2D.Input.HORIZONTAL);
                _motor.normalizedYMovement = UnityEngine.Input.GetAxis(PC2D.Input.VERTICAL);

                return; // do nothing more
            }

            // X axis movement
            if (Mathf.Abs(UnityEngine.Input.GetAxis(PC2D.Input.HORIZONTAL)) > PC2D.Globals.INPUT_THRESHOLD)
            {
                _motor.normalizedXMovement = UnityEngine.Input.GetAxis(PC2D.Input.HORIZONTAL);
            }
            else
            ...
            if (UnityEngine.Input.GetAxis(PC2D.Input.VERTICAL) != 0)
            {
                bool up_pressed = UnityEngine.Input.GetAxis(PC2D.Input.VERTICAL) > 0;
            ...
            else if (UnityEngine.Input.GetAxis(PC2D.Input.VERTICAL) < -PC2D.Globals.FAST_FALL_THRESHOLD)
            {
                _motor.fallFast = false;
            }

            if (UnityEngine.Input.GetButtonDown(PC2D.Input.DASH))
            {
                _motor.Dash();
            }
        }
